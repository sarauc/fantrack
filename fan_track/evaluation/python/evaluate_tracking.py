...
class trackingEvaluation(object):
    ...
    def __init__(self, t_sha, gt_path="./data/tracking", min_overlap=0.5, max_truncation = 0, min_height = 25, max_occlusion = 2, mail=None, cls="car", mapping_path="./data/tracking/evaluate_tracking.seqmap"):
        ...
        self.t_sha             = t_sha
        self.t_path            = '../../data/predictions' + '/' + t_sha + '/'
    ...
    def createSummary(self):
        ...
        accuracy_dict[self.sequence_name[0]] = self.MOTA
...
if __name__ == "__main__":
    actual_file =  "./data/tracking/evaluate_tracking.seqmap"
    
    parser = argparse.ArgumentParser()
    parser.add_argument('--fr', type=bool, default=True,help='full results')
    parser.add_argument('--seq_type', type=str, default='validation', help = 'Type of sequence')
    parser.add_argument('--root_dir', type=str, default='training', help='root folder of results')
    args = parser.parse_args()
    
    full_results = args.fr
    result_sha = args.root_dir

    filename_test_mapping = "./data/tracking/evaluate_tracking.seqmap." + args.seq_type
    ...
    if full_results:
        ...
        with open(filename_test_mapping, "r") as fh:
            with open(actual_file, "w") as afh:
                for i,l in enumerate(fh):
                    # Write the test line to file
                    afh.write(l)
                afh.close()
            fh.close()
        ...
        # Batch run evaluating individually
        with open(filename_test_mapping, "r") as fh:
            for i,l in enumerate(fh):
    ...
    for item in range(0,21):
        itemstr = "%04d" % item
        if itemstr in accuracy_dict:
            print('Video:'+ str(item) + ' '+ "%.2f"%accuracy_dict[itemstr])
